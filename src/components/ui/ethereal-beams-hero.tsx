import React, { forwardRef, useImperativeHandle, useEffect, useRef, useMemo, FC, ReactNode, useState } from "react"
...
export const Button = ({ variant = "default", size = "sm", className = "", children, ...props }: ButtonProps) => {
  const baseClasses =
    "inline-flex items-center justify-center font-medium transition-all duration-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/20 disabled:pointer-events-none disabled:opacity-50 active:scale-95 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.8)] backdrop-blur-[40px] hover:-translate-y-1 group relative overflow-hidden rounded-full"

  const variants = {
    default: "bg-white/[0.08] text-white border border-white/[0.2] hover:bg-white/[0.15] hover:border-white/[0.3] hover:shadow-[0_20px_40px_rgba(255,255,255,0.1),inset_0_1px_1px_rgba(255,255,255,0.4)]",
    outline: "bg-white/[0.02] text-white/90 border border-white/[0.1] hover:bg-white/[0.08] hover:text-white hover:border-white/[0.25] hover:shadow-[0_0_20px_rgba(255,255,255,0.05),inset_0_1px_1px_rgba(255,255,255,0.2)]",
    ghost: "text-white/80 hover:text-white hover:bg-white/[0.08] border border-transparent hover:border-white/[0.1] hover:shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)]",
  }

  const sizes = {
    sm: "h-10 px-6 py-2 text-sm tracking-wide",
    lg: "h-14 px-10 py-3 text-lg tracking-wide",
  }
