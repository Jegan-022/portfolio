...
const SendButton = ({ disabled, isSubmitting, isSubmitted, onClick }: { disabled?: boolean, isSubmitting?: boolean, isSubmitted?: boolean, onClick?: (e: any) => void }) => {
  return (
    <StyledWrapper style={{ opacity: disabled && !isSubmitting && !isSubmitted ? 0.7 : 1, pointerEvents: disabled ? 'none' : 'auto' }}>
      <button className={`button ${isSubmitting ? 'is-submitting' : ''} ${isSubmitted ? 'is-submitted' : ''}`} type="submit" disabled={disabled} onClick={onClick}>
...
const StyledWrapper = styled.div`
  ...
  .button.is-submitted .state--default svg {
    animation: takeOff 0.8s linear forwards;
  }
  .button.is-submitting .state--default .icon,
  .button.is-submitted .state--default .icon {
  ...
  .button.is-submitted .state--default .icon:before {
  ...
  .button.is-submitting .state--default,
  .button.is-submitted .state--default {
    position: absolute;
  }
  
  .button.is-submitting .state--loading {
    display: flex;
  }
  .button.is-submitting .state--loading span {
    opacity: 0;
    animation: slideDown 0.8s ease forwards calc(var(--i) * 0.1s);
  }

  .button.is-submitted .state--sent {
    display: flex;
  }
  .button.is-submitted .state--sent span {
    opacity: 0;
    animation: slideDown 0.8s ease forwards calc(var(--i) * 0.2s);
  }
  .button.is-submitted .state--sent .icon svg {
    opacity: 0;
